...
bool FilmDesign::GetFileInfo(string filePath)
{
    bool bRet = false;
    try
    {
...
bool FilmDesign::GetFileInfo1(string filePath)
{
    bool bRet = false;
    try
    {
...
bool FilmDesign::GetFileInfo2(string filePath)
{
    bool bRet = false;
    try
    {
...
void FilmDesign::TransRefQWtoMonitorQW(unsigned int layerItem)
{
    ...
    {
        double layerN = GetLayerN(i+1);
        double monitorN = GetLayerRefWLN(i+1);
...
//分割字符串
vector<string> FilmDesign::Split(string str,string pattern)
{
...
//初始化parameters
vector<double> FilmDesign::InitiateParameter()
{
    ...
    {
        case 5:
            checkParams.push_back(layerQW[curLayer-2]);
            break;
        case 6:
        {
            checkParams.push_back(layerQW[curLayer-2]);
            checkParams.push_back(layerQW[curLayer-3]);
            break;
        }
        case 7:
        {
            checkParams.push_back(layerQW[curLayer-2]);
            checkParams.push_back(layerQW[curLayer-3]);
            checkParams.push_back(layerQW[curLayer-4]);
            break;
        }
        default:
            break;
...
vector<double> FilmDesign::GetParamter(vector<double> parameters)
{
    vector<double>().swap(checkParams);
    checkParams = parameters;
    return checkParams;
}
void FilmDesign::ChangeParasToLayerInfo(vector<double> parameters)
{
...
//        LN = parameter[2];
     ...
     {
         case 5:
             layerQW[curLayer - 2] = parameters[4];
             break;
         case 6:
             layerQW[curLayer - 2] = parameters[4];
             layerQW[curLayer - 3] = parameters[5];
             break;
         case 7:
             layerQW[curLayer - 2] = parameters[4];
             layerQW[curLayer - 3] = parameters[5];
             layerQW[curLayer - 4] = parameters[6];
             break;
         default:
             break;
     }
...
*/
    ...
    return (deltaCurQW)*PenaltyLambda;
}

//根据parameters和时间t所对应的透过率,用于模拟优化的,在这基础上加上折射率的惩罚函数
double FilmDesign::ComputeT1(double t, vector<double> parameters)
{
    ChangeParasToLayerInfo(parameters);
    return ComputeCurLight(t);
}
//计算对应膜层的Matrix
CoatMatrix FilmDesign::ComputeLayerMat(unsigned int layerItem)
{
...
//获取对应膜层的折射率
double FilmDesign::GetLayerN(unsigned int layerItem)
{
...
//获取对应膜层在RefWL时的折射率
double FilmDesign::GetLayerRefWLN(unsigned int layerItem)
{
    if (layerMat[layerItem - 1] == HName)
    {
        return HN_Original;
...
//获取对应膜层的光学厚度
double FilmDesign::GetLayerQW(unsigned int layerItem)
{
    return layerQW[layerItem - 1];
}
//获取对应膜层的速率
double FilmDesign::GetLayerRate(unsigned int layerItem)
{
    if (layerMat[layerItem - 1] == HName)
     {
         return HRate;
...
//获取对应膜层的控制方式
string FilmDesign::GetLayerControlType(unsigned int layerItem)
{
    return layerControlType[layerItem-1];
}
//计算不用于优化的膜层组的Matrix
CoatMatrix FilmDesign::ComputeNoOptiMat()
{
    ...
    {
        totalMatrix = ComputeLayerMat(curLayer - 7);
        for (unsigned int i = (curLayer - 7);i > 0; i--)
        {
            matrix1 = totalMatrix;
...
CoatMatrix FilmDesign::ComputePreCurLayerMat()
{
    CoatMatrix totalMatrix ;
    CoatMatrix matrix1 ;
    CoatMatrix matrix2 ;
    totalMatrix = ComputeLayerMat(curLayer - 1);
    if (preNoOptiMatrix.isNull()) //如果未经过计算Matrix则计算一下
    {
        ComputeNoOptiMat();
    }
    if (curLayer > 7)
    {
        for (unsigned int i = (curLayer - 1); i > (curLayer - 6); i--)
        {
            matrix1 = totalMatrix;
            matrix2 = ComputeLayerMat(i - 1);
            totalMatrix = matrix1* matrix2;
        }
        preTotalMatrix = totalMatrix* preNoOptiMatrix;
    }
    else
    {
...
CoatMatrix FilmDesign::GetCurLayerMatrix(double t)
{
    //获取单前层的折射率
    CoatMatrix tempCoatMatrix;
...
//计算时间点t的透过率
double FilmDesign::ComputeCurLight(double t)
{
    CoatMatrix totalMatrix = GetCurLayerMatrix(t)* preTotalMatrix;
    return ComputeMatrixLight(totalMatrix);
}
//计算CoatMatrix的透过率
double FilmDesign::ComputeMatrixLight(CoatMatrix totalMatrix)
{
...
//计算thkQW对应的透过率
double FilmDesign::GetCurThkLightVal(double thkQW)
{
    CoatMatrix tempCoatMatrix;
    double n = GetLayerN(curLayer);
    //获取单前层的速率
...
//计算thkQW对应的相位
double FilmDesign::GetCurThkPhaseVal(double thkQW)
{
    CoatMatrix tempCoatMatrix ;
    double n = GetLayerN(curLayer);
    //获取单前层的速率
...
//计算CutPeak对应QW
double FilmDesign::GetNextPeakQW()
{
    if (preTotalMatrix.isNull())
    {
        ComputePreCurLayerMat();
...
void FilmDesign::GetInfoVal(double& CutPeakQW,int& nCutPeak,double& Tmax,double&Tmin, double& Tstart,double& Tend,double& Ph_end,double& Tprepeak)
{
    ComputePreCurLayerMat();
...
void FilmDesign::GetMaxMinValue(double d1,double d2,double d3,double d4,double& dMax,double& dMin)
{
    dMax = d1;
    dMin = d1;
    if(dMax < d2)
...
void FilmDesign::GetMaxMinValue(double d1,double d2,double d3,double& dMax,double& dMin)
{
    dMax = d1;
    dMin = d1;
    if(dMax < d2)
...
double FilmDesign::GetSubstrateN(string substrateFileName)
{
    try
    {
        QString FileAddress = QCoreApplication::applicationDirPath();
        QString logFileName = FileAddress + "//" + QString::fromStdString(subStrateName) + ".txt";
        QFile f(logFileName);
...
double FilmDesign::GetLayerN(string layerFileName,double WL)
{
    try
    {
        QString FileAddress = QCoreApplication::applicationDirPath();
