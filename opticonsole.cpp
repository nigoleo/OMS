...
void OptiConsole::GetFittingInfo(double &QW_Phase,double &QW_Offset,double &QW_CutPeak,
                               ...
                               double &fittingN,double &fittingRate,double &X2,vector<double> &SimulateT,vector<double> &test)
{
    //计算理论设计当前层的相关数据
    double cutPeakQWNext_theory = 0;     //第一个拐点对应的QW
    int nCutPeak_theory = 0;             //拐点数
    double Tend_theory = 0;              //结束透过率
    double cutNextPeakQW_theory = 0;     //本层结束后继续镀膜达到第一个拐点的QW
    double TnextPeak_theory = 0;         //完成本层后如果继续镀膜达到第一个峰值的透过率
    double TsecondPeak_theory = 0;       //完成本层后如果继续镀膜达到第二个峰值的透过率
    double qw = TheoryDesign->GetLayerQW(TheoryDesign->GetCurLayer());  //本层需要完成的QW
    TheoryDesign->GetInfoVal1(cutPeakQWNext_theory,nCutPeak_theory,Tend_theory);
    TheoryDesign->GetInfoVal2(qw,cutNextPeakQW_theory,TnextPeak_theory,TsecondPeak_theory);
    //计算模拟数据的当前层
    double cutPeakQWNext_act = 0;     //第一个拐点对应的QW
    int nCutPeak_act = 0;             //拐点数
    double Tend_act = 0;              //结束透过率
    double cutNextPeakQW_act = 0;     //本层结束后继续镀膜达到第一个拐点的QW
    double TnextPeak_act = 0;         //完成本层后如果继续镀膜达到第一个峰值的透过率
    double TsecondPeak_act = 0;       //完成本层后如果继续镀膜达到第二个峰值的透过率
    ...
    SimulateDesign->GetInfoVal1(cutPeakQWNext_act, nCutPeak_act, Tend_act);
    ...
    SimulateDesign->GetInfoVal2(QW_CutPeak,cutNextPeakQW_act,TnextPeak_act,TsecondPeak_act);

    Tend_Offset_theory = Tend_theory-TsecondPeak_theory+TsecondPeak_act;
    double Tend_Phase = (Tend_theory-TsecondPeak_theory)/(TnextPeak_theory-TsecondPeak_theory)*(TnextPeak_act-TsecondPeak_act)+TsecondPeak_act;
    test.push_back(Tend_Phase);
    test.push_back(Tend_theory);
    test.push_back(TnextPeak_theory);
    test.push_back(TsecondPeak_theory);
    test.push_back(TnextPeak_act);
    test.push_back(TsecondPeak_act);


    double TScale = TnextPeak_act-TsecondPeak_act;
...
//    double eps1 = 1e-14;
    ...
    while (true)
    {
        ...
        if((abs(SimulateDesign->GetCurThkLightVal(QW_Offset)-Tend_Offset_theory))/abs(TScale)<0.001)
        {
            break;
        }
    ...
    //计算Phase控制方式所对应的QW和T%end
    //利用GradientDescent方式求Phase的QW
    stepQW = 0.0001;  //learningRate
//    double eps1 = 1e-14;
    QW_Phase = QW_CutPeak;
    PreAlpha = GetGradientOffset(QW_Phase,Tend_Phase);
    nCount = 0;
    alpha = PreAlpha;
    while (true)
    {
        ...
        nCount++;
        alpha = GetGradientOffset(QW_Phase,Tend_Phase);
        ...
        if((abs(SimulateDesign->GetCurThkLightVal(QW_Phase)-Tend_Phase))/abs(TScale)<0.001)
        {
            break;
        }
    ...
    Tend_Phase_act = SimulateDesign->GetCurThkLightVal(QW_Phase);         //Offset方式实际完成的透过率
    Phend_act = SimulateDesign->GetCurThkPhaseVal(QW_Phase);                             //phase方式实际完成的相位
    Pend_theory = TheoryDesign->GetCurThkPhaseVal(TheoryDesign->GetLayerQW(TheoryDesign->GetCurLayer()));
    DeltaPhase = abs(Phend_act-Pend_theory);
