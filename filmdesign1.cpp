#include "filmdesign1.h"

FilmDesign1::FilmDesign1()
{
    isNoNull = false;
...
bool FilmDesign1::GetFileInfo(string filePath)
{
    bool bRet = false;
    try
    {
...
bool FilmDesign1::GetFileInfo1(string filePath)
{
    bool bRet = false;
    try
    {
...
bool FilmDesign1::GetFileInfo2(string filePath)
{
    bool bRet = false;
    try
    {
...
void FilmDesign1::TransRefQWtoMonitorQW(unsigned int layerItem)
{
...
//分割字符串
vector<string> FilmDesign1::Split(string str,string pattern)
{
...
//初始化parameters
vector<double> FilmDesign1::InitiateParameter()
{
...
vector<double> FilmDesign1::GetParamter(vector<double> parameters)
{
    vector<double>().swap(checkParams);
    checkParams = parameters;
    return checkParams;
}
void FilmDesign1::ChangeParasToLayerInfo(vector<double> parameters)
{
...
double FilmDesign1::ComputeT1(double t, vector<double> parameters)
{
    ChangeParasToLayerInfo(parameters);
    return ComputeCurLight(t);
}
//计算对应膜层的Matrix
CoatMatrix FilmDesign1::ComputeLayerMat(unsigned int layerItem)
{
...
//获取对应膜层的折射率
double FilmDesign1::GetLayerN(unsigned int layerItem)
{
...
//获取对应膜层在RefWL时的折射率
double FilmDesign1::GetLayerRefWLN(unsigned int layerItem)
{
    if (layerMat[layerItem - 1] == HName)
    {
        return HN_Original;
...
//获取对应膜层的光学厚度
double FilmDesign1::GetLayerQW(unsigned int layerItem)
{
    return layerQW[layerItem - 1];
}
//获取对应膜层的速率
double FilmDesign1::GetLayerRate(unsigned int layerItem)
{
    if (layerMat[layerItem - 1] == HName)
     {
         return HRate;
...
//获取对应膜层的控制方式
string FilmDesign1::GetLayerControlType(unsigned int layerItem)
{
    return layerControlType[layerItem-1];
}
//计算不用于优化的膜层组的Matrix
CoatMatrix FilmDesign1::ComputeNoOptiMat()
{
...
CoatMatrix FilmDesign1::ComputePreCurLayerMat()
{
    CoatMatrix totalMatrix ;
    CoatMatrix matrix1 ;
    CoatMatrix matrix2 ;
    ...
    if (curLayer > 4)
    {
        for (unsigned int i = (curLayer - 1); i > (curLayer - 3); i--)
        {
            matrix1 = totalMatrix;
            matrix2 = ComputeLayerMat(i - 1);
            totalMatrix = matrix1* matrix2;
...
CoatMatrix FilmDesign1::GetCurLayerMatrix(double t)
{
    //获取单前层的折射率
    CoatMatrix tempCoatMatrix;
...
//计算时间点t的透过率
double FilmDesign1::ComputeCurLight(double t)
{
    CoatMatrix totalMatrix = GetCurLayerMatrix(t)* preTotalMatrix;
    return ComputeMatrixLight(totalMatrix);
}
//计算CoatMatrix的透过率
double FilmDesign1::ComputeMatrixLight(CoatMatrix totalMatrix)
{
...
//计算thkQW对应的透过率
double FilmDesign1::GetCurThkLightVal(double thkQW)
{
    CoatMatrix tempCoatMatrix;
    double n = GetLayerN(curLayer);
    //获取单前层的速率
...
//计算thkQW对应的相位
double FilmDesign1::GetCurThkPhaseVal(double thkQW)
{
    CoatMatrix tempCoatMatrix ;
    double n = GetLayerN(curLayer);
    //获取单前层的速率
...
//计算CutPeak对应QW
double FilmDesign1::GetNextPeakQW()
{
    double DR,DI,ER,EI;
    double aa,bb,cc,dd;
    double a,b,c;
...
void FilmDesign1::GetInfoVal(double& CutPeakQW,int& nCutPeak,double& Tmax,double&Tmin, double& Tstart,double& Tend,double& Ph_end,double& Tprepeak)
{
...
void FilmDesign1::GetMaxMinValue(double d1,double d2,double d3,double d4,double& dMax,double& dMin)
{
    dMax = d1;
    dMin = d1;
    if(dMax < d2)
...
void FilmDesign1::GetMaxMinValue(double d1,double d2,double d3,double& dMax,double& dMin)
{
    dMax = d1;
    dMin = d1;
    if(dMax < d2)
...
double FilmDesign1::GetSubstrateN(string substrateFileName)
{
...
double FilmDesign1::GetLayerN(string layerFileName,double WL)
{
    try
    {
        QString FileAddress = QCoreApplication::applicationDirPath();
